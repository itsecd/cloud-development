...
/// <summary>
/// Служба для манипуляции файлами с пациентами в объектном хранилище Minio.
/// </summary>
/// <param name="client">S3-клиент Minio.</param>
/// <param name="configuration">Конфигурация.</param>
/// <param name="logger">Логгер.</param>
public sealed class S3MinioService(
    IMinioClient client,
    ...
    ILogger<S3MinioService> logger) : IS3Service
{
    ...
    public async Task<bool> UploadFile(string fileData)
    {
        ...
        var request = new PutObjectArgs()
            ...
            .WithObject($"patient_{id}.json");

        var response = await client.PutObjectAsync(request);

        if (response.ResponseStatusCode != HttpStatusCode.OK)
        {
            logger.LogError("Failed to upload patient {file}: {code}", id, response.ResponseStatusCode);
            return false;
        }
