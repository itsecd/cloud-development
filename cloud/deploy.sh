...
rm -f "$API_ZIP"
(cd "$ROOT_DIR/ProjectApp.Api.Function" && zip -q "$API_ZIP" *.cs *.csproj)
...
yc serverless function version create \
  ...
  --runtime dotnet8 \
  --entrypoint "ProjectApp.Api.Function.Handler" \
  --memory 256m \
...
rm -f "$FILE_ZIP"
(cd "$ROOT_DIR/ProjectApp.FileService.Function" && zip -q "$FILE_ZIP" *.cs *.csproj)
...
yc serverless function version create \
  ...
  --folder-id "$YC_FOLDER_ID" \
  --runtime dotnet8 \
  --entrypoint "ProjectApp.FileService.Function.Handler" \
  --memory 256m \
